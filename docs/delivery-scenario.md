...
**실패(X · 혼동 · 인식실패)** 면 로봇이 병실 자리에 정지한 채 간호사에게 알림을 보내고, 간호사는 두 갈래 중 하나를 선택합니다 — **"바로 복귀"** 를 누르면 😢 슬픔 표정 후 로봇이 혼자 돌아오고, **"대기해, 내가 갈게"** 를 누르면 로봇이 병실 그 자리에서 최대 5분 대기하다가 🚶 간호사가 도착하면 함께 걸어서 간호실로 돌아옵니다. 5분이 지나도 아무 응답이 없으면 로봇이 혼자 자동 복귀합니다.
...
간호사가 화면에서 **방 번호(102/103/104)** 와 **물품**을 카드처럼 선택하고 "요청" 버튼을 누릅니다.
(101호는 간호실 — 로봇의 대기·복귀 지점이라 배송 대상이 아닙니다.)
...
로봇은 **병실 도착 지점 그대로 정지한 채 대기**합니다. 간호사의 결정을 기다리며, 5분 안에는 스스로 복귀하지 않습니다.
...
- **5분이 지나도 간호사 응답이 없으면**, 로봇이 무한정 병실을 점유하지 않도록 관제 시스템이 배송을 "실패"로 확정하고 **로봇을 혼자 자동 복귀**시킵니다. 실패 사유는 처음 판정된 사유(예: "실패 카드 감지")를 그대로 유지합니다.
- 5분 경과 시 간호사에게 재알림을 보낼지, 다른 간호사에게 에스컬레이션할지는 §6 미결정.
...
간호사가 **102호 · "물건"** 을 요청 → 로봇이 102호로 이동 → 도착 후 30초 동안 카드 확인 → **성공(O)** 감지 → 화면에 성공 알림, 로봇 LCD에 **웃는 얼굴** → 간호실 복귀.
...
7. **5분 초과 시 알림 정책** — 자동 복귀 자체는 확정됐지만(아래), 그 전에 간호사에게 알림을 재전송할지 · 다른 간호사에게 에스컬레이션할지는 미정.
8. **함께 복귀 시작 트리거** — 간호사가 병실에 도착한 사실을 로봇/시스템이 어떻게 인지할지 (앱 버튼 / 로봇 자체 버튼 / 자동 인식).

### ✅ 확정된 것

- **실패 시 즉시 자동 복귀 안 함** — 로봇은 간호사의 선택("바로 복귀" / "대기해, 내가 갈게") 을 기다림.
- **대기 위치** — 로봇이 병실에 도착한 그 좌표(움직이지 않음).
- **대기 상한** — **5분**. 초과하면 배송을 "실패"로 확정하고 로봇을 혼자 자동 복귀시킴 (무한 대기 방지).
- **"대기" 이후 흐름** — 5분 안에 간호사가 오면 로봇 혼자 복귀하지 않고 **간호사와 함께 간호실로 복귀**.
...
| 위치                  | 비고                          |
| --------------------- | ----------------------------- |
| 간호실 (101호 · 원점) | 로봇의 대기·복귀 지점         |
| 102호 · 103호 · 104호 | 데모용 병실 3곳 (배송 목적지) |
